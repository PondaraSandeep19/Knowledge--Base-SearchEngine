...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Evaluation endpoint: accepts labeled queries and returns precision@k
  app.post("/api/evaluate", async (req, res) => {
    try {
      const { examples, k = 3 } = req.body;

      // examples: Array<{ query: string, relevantDocumentIds: string[] }>
      ...
      for (const ex of examples) {
        const { query, relevantDocumentIds } = ex;
        if (!query || !Array.isArray(relevantDocumentIds)) continue;

        const queryEmbedding = await generateEmbedding(query);
        const { precision, retrieved } = require("./utils/evaluate").precisionAtK(
          queryEmbedding,
          allChunks as any,
          new Set(relevantDocumentIds),
          k
        );

        results.push({ query, precision, retrieved });
      }

      // Compute average precision
      const avgPrecision = results.reduce((s, r) => s + r.precision, 0) / results.length || 0;

      res.json({ results, avgPrecision });
    } catch (error) {
